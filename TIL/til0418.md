...
호출 형태에 따라서 person.introduce(); 
bind this를 바꿔칠 수 도 있었는데( 변수에 저장하여? ) 화살표함수에는 이런 성질이 없다. 

화살표함수에서 this가 무엇을 가리킬까 방식은 function과는 다르다 
함수가 정의된 스코프에서 정의된 스코프에 존재하는 this를 가리킨다. 
바깥쪽  this와 똑같은 this가 됨 ???

화살표함수는 생성될 때  this가 결정됨 
어떻게 사용되건, 호출되건, 바뀌지 않음 
this.getname 의 this 임 
fucntion 은 호출되는 상황에 따라서 

function  어떻게 호출되느냐에 따라서 this가 결정되고 
화살표함수는 어떻게 정의되느냐, 생성되느냐에 따라 this가 결정됨 
